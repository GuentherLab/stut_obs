function testMicHeadphones(proj, varargin)
close all;
% proj: 'SEQM' or 'SEQS'
%
% varargin:
...
if ~any(find(strcmp({'SEQM', 'SEQS'}, proj)))
    disp('Unrecognized project identifier (should be "SEQM" or "SEQS")');
else
    expParams.proj = proj;
end
...
switch proj
    case 'SEQM'
        stipath = fullfile(expParams.audiopath, 'practice', 'GEIN-CHER-NOI.wav');
        [Input_sound{1}, Input_fs{1}]=audioread(stipath);
        Input_conditions = {'GEIN-CHER-NOI'};
    case 'SEQS'
        stipath = fullfile(expParams.audiopath, 'practice', 'vgukp.wav');
        [Input_sound{1}, Input_fs{1}]=audioread(stipath);
        Input_conditions = {'VGUKP'};
end
...
nexttrl=uicontrol(rtfig,'style','pushbutton','string','Skip to Next','units','norm','position',[.55,.01,.1,.05],'callback',@nxtTrial,'fontsize',default_fontsize-1);
conti=uicontrol(rtfig,'style','pushbutton','string','Next','units','norm','position',[.7,.01,.1,.05],'callback','uiresume','fontsize',default_fontsize-1);
cance=uicontrol(rtfig,'style','pushbutton','string','End','units','norm','position',[.85,.01,.1,.05],'callback','delete(gcbf)','fontsize',default_fontsize-1);
set(nexttrl,'visible','off');
set(conti,'visible','off');
set(cance,'visible','off');
