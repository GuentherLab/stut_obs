...
fnames=fnames(~ismember(fnames,{'visual', 'root', 'textpath', 'figurespath', 'subject', 'session', 'run', 'task', 'gender', 'scan', 'deviceMic','deviceHead','deviceScan'}));
...
out_dropbox = {'visual', 'root', 'textpath', 'figurespath', 'subject', 'session', 'run', 'task', 'gender', 'scan'};
...
% locate files
% "C:\Users\splab\Documents\SIT-Pilot\sub-SITpilot01\ses-1\test\sub-SITpilot01_ses-1_run-1_task-test_desc-conditions.txt"
% C:\Users\splab\Documents\SIT-Pilot\sub-TEST01\ses-1\beh\test\sub-TEST01_ses-1_run-1_task-test_desc-stimulus.txt
...
[silent_sound,silent_fs]=audioread(fullfile(expParams.textpath, 'silent.wav'));
...
silence_dur = size(silent_sound,1) / silent_fs;    
silenceLen  = ceil(44100 * silence_dur);
silenceVec  = zeros(silenceLen, 1);

nTrials = numel(Input_files);
[Input_sound{1:nTrials}] = deal(silenceVec);
[Input_fs{1:nTrials}] = deal(44100);
